...
class FbSample_ExtraCustomerField extends Module
{
    public function __construct()
    {
        ...
        $this->author = 'Alexander marcondes';
        $this->version = '1.1.0';
    ...
    {
        Db::getInstance()->execute('ALTER TABLE `'. _DB_PREFIX_.'customer` ADD `additionalInfo` text');
        return true;
    }
    ...
    {
        $additionalInfo = Tools::getValue('additionalInfo');
        $query = 'UPDATE `'._DB_PREFIX_.'customer` c '
            .' SET  c.`additionalInfo` = "'.pSQL($additionalInfo).'"'
            .' WHERE c.id_customer = '.(int)$id_customer;
    ...
    public function hookAdditionalCustomerFormFields($params)
    {
        ...
        $additionalInfo_value = Tools::getValue('additionalInfo');
        if (isset($module_fields['additionalInfo'])) {
            $additionalInfo_value = $module_fields['additionalInfo'];
        }

        $extra_fields = array();
        $extra_fields['additionalInfo'] = (new FormField)
            ->setName('additionalInfo')
            ->setType('text')
            ->setValue($additionalInfo_value)
            ->setLabel($this->l('additionalInfo'));
